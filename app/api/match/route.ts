  ...
  try {
    ...
    const candidatePartners = partners.filter((partner: any) => {
      ...
      if (comparisonAmount > 0 && (comparisonAmount < minCheck * strictnessMultiplier || comparisonAmount > maxCheck * upperMultiplier)) {
        ...
        console.log(`  ❌ Filtered out ${partner.name}: Check size mismatch (${availableLabel}, range: $${minCheck.toLocaleString()}-${maxCheck === Infinity ? "∞" : `$${maxCheck.toLocaleString()}`}, strictness: ${checkSizeFilterStrictness}%)`);
        return false;
      }
