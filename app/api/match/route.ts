...
export async function POST(request: Request) {
  try {
    ...
    if (!deal || !partners || partners.length === 0) {
      return NextResponse.json(
        { error: "Deal and partners are required" },
        { status: 400 }
      );
    }

    console.log(`\n=== MATCHING DEAL: ${deal.name} ===`);
    console.log(`Against ${partners.length} partners`);
    ...
    // Assess deal data quality
    const dealDataQuality = assessDataQuality(deal, { thesis: "", investmentSpace: "", investmentStage: "", checkSize: "" });
    console.log(`Deal data quality: ${dealDataQuality.dealQuality}%`);
    if (dealDataQuality.dealWarnings.length > 0) {
      console.log(`⚠️  Deal data warnings: ${dealDataQuality.dealWarnings.join(", ")}`);
    }

    // Level 1: Configurable Hard Filters (Quick elimination of obvious mismatches)
    const candidatePartners = partners.filter((partner: any) => {
      // Parse deal amount
      ...
      if (dealAmount > 0 && (dealAmount < minCheck * strictnessMultiplier || dealAmount > maxCheck * upperMultiplier)) {
        console.log(`  ❌ Filtered out ${partner.name}: Check size mismatch (deal: $${dealAmount.toLocaleString()}, range: $${minCheck.toLocaleString()}-$${maxCheck === Infinity ? "∞" : maxCheck.toLocaleString()}, strictness: ${checkSizeFilterStrictness}%)`);
        return false;
      }

      // Apply data quality filter
      const quality = assessDataQuality(deal, partner);
      if (quality.partnerQuality < minDataQuality) {
        console.log(`  ❌ Filtered out ${partner.name}: Data quality too low (${quality.partnerQuality}% < ${minDataQuality}%)`);
        return false;
      }
    ...
    console.log(`After hard filters: ${candidatePartners.length} candidates`);

    // Level 2: AI-Powered Multi-Factor Evaluation
    const matches = await Promise.all(
      candidatePartners.slice(0, 50).map(async (partner: any) => {
        try {
...
Respond ONLY with valid JSON (no markdown, no code blocks):
{
  ...
  "dealbreakers": ["<critical mismatch>"] or []
          ...
          return {
            ...
            investmentStage: partner.investmentStage,
            investmentSpace: partner.investmentSpace,
            regions: partner.regions,
          ...
          console.error(`Error matching ${partner.name}:`, error);
          return {
            partnerId: partner.id,
            partnerName: partner.name,
            score: 0,
    ...
    const goodMatches = matches
      .filter((m) => {
        ...
        if (m.dealbreakers && m.dealbreakers.length > 0) {
          console.log(`  ❌ Filtered out ${m.partnerName}: Dealbreakers found - ${m.dealbreakers.join(", ")}`);
          return false;
        }
    ...
    const avgDealQuality = dealDataQuality.dealQuality;
    const avgPartnerQuality = candidatePartners.length > 0 
      ? candidatePartners.reduce((sum: number, p: any) => {
          const q = assessDataQuality(deal, p);
          return sum + q.partnerQuality;
        }, 0) / candidatePartners.length
      : 0;

    return NextResponse.json({
      matches: goodMatches,
      totalEvaluated: candidatePartners.length,
      totalMatches: goodMatches.length,
      dataQuality: {
        deal: avgDealQuality,
        partners: Math.round(avgPartnerQuality),
        warnings: dealDataQuality.dealWarnings,
        recommendation: avgDealQuality < 50 || avgPartnerQuality < 50 
          ? "Consider adding more detail to deal descriptions and VC investment theses for better matches"
    ...
    return NextResponse.json(
      { error: error.message || "Failed to match deal" },
      { status: 500 }
    );
  }
}
